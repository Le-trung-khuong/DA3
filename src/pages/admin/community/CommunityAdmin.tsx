/**
 * Smart Review — Admin Community Management (Firestore Realtime)
 * File: src/pages/admin/community/CommunityAdmin.tsx
 * FIX: xử lý lastMessageAt là Timestamp từ Firestore
 * FIX: lấy roomId từ collectionGroup messages (doc.ref.parent.parent?.id)
 */
...
import { orderBy, where, limit, collectionGroup, onSnapshot, query as firestoreQuery, doc, updateDoc, deleteDoc, addDoc, serverTimestamp, increment } from "firebase/firestore";
...
import {
  ...
  Hash, Lock, Unlock, Bell, Clock,
  CheckCircle, AlertOctagon, Info,
  ArrowLeft, Filter,
  UserX, Ban, ShieldAlert, BarChart2,
  MessageCircle, Zap, Save, Send,
} from "lucide-react";
...
// Chuyển Timestamp hoặc Date thành milliseconds
  ...
  return (
    <div
      style={{
        ...
        border: `1px solid ${room.reportedCount > 0 ? "rgba(255,180,171,.2)" : "rgba(255,255,255,.06)"}`,
        borderRadius: 18,
        padding: "16px 18px",
      ...
      onMouseOver={(e) =>
        (e.currentTarget.style.borderColor = room.reportedCount > 0 ? "rgba(255,180,171,.35)" : "rgba(255,255,255,.12)")
      }
      onMouseOut={(e) =>
        (e.currentTarget.style.borderColor = room.reportedCount > 0 ? "rgba(255,180,171,.2)" : "rgba(255,255,255,.06)")
      }
    >
      {room.pinned && (
        <div
            ...
            {!room.isActive && (
              <span
                style={{
                  padding: "2px 8px",
                  borderRadius: 999,
                  background: "rgba(176,174,192,.12)",
                  border: "1px solid rgba(176,174,192,.25)",
                  fontSize: 10,
                  fontWeight: 700,
                  color: "#B0AEC0",
                }}
            ...
            {menuOpen && (
              ...
              >
                {[
                  ...
                  {
                    Icon: room.isLocked ? Unlock : Lock,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <h2 style={{ fontSize: 17, fontWeight: 800, color: "#E4E1EE" }}>
              {isEdit ? "Edit Room" : "Create Room"}
            </h2>
            <p style={{ fontSize: 11, color: "#9B59B6" }}>
              {isEdit ? `Firestore: chat_rooms/${room!.id}` : "Adds doc to chat_rooms collection"}
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    transition: "all .15s",
                    background: on ? `${colorOn}18` : "rgba(255,255,255,.04)",
                    border: `1px solid ${on ? colorOn + "40" : "rgba(255,255,255,.08)"}`,
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              background: form.name.trim() ? "linear-gradient(135deg,#6C63FF,#9B59B6)" : "rgba(255,255,255,.04)",
              border: "none",
              color: form.name.trim() ? "#fff" : "#47464f",
              boxShadow: form.name.trim() ? "0 0 18px rgba(108,99,255,.3)" : "none",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              gap: 8,
          ...
          >
            {saving ? (
              ...
              <>
                <Save size={14} /> {isEdit ? "Save changes" : "Create room"}
              </>
            )}
          </button>
        </div>
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              cursor: "pointer",
              background: showReported ? "rgba(255,180,171,.15)" : "rgba(255,255,255,.05)",
              border: `1px solid ${showReported ? "rgba(255,180,171,.3)" : "rgba(255,255,255,.09)"}`,
              color: showReported ? "#ffb4ab" : "#C7C4D8",
            }}
          >
            ...
            displayed.map((msg) => (
              <div
                ...
                style={{
                  ...
                  padding: "10px 12px",
                  borderRadius: 12,
                  background: msg.isReported ? "rgba(255,180,171,.08)" : "rgba(255,255,255,.025)",
                  border: `1px solid ${msg.isReported ? "rgba(255,180,171,.22)" : "rgba(255,255,255,.06)"}`,
                }}
              >
                <div
                  style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <span style={{ fontSize: 12, fontWeight: 800, color: msg.isReported ? "#ffb4ab" : "#E4E1EE" }}>
                      {msg.userName}
                    </span>
                    {msg.isReported && (
                      <span
                  ...
                  <p style={{ fontSize: 13, color: msg.isReported ? "#ffb4ab" : "#C7C4D8", lineHeight: 1.5 }}>
                    {msg.text}
                  </p>
                </div>
                {msg.isReported && (
                  <button
                    onClick={() => onDeleteMsg(msg)}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 style={{ fontSize: 18, fontWeight: 700, color: "#E4E1EE", textAlign: "center", marginBottom: 10 }}>
          Delete Message?
        </h2>
        <p style={{ fontSize: 13, color: "#C7C4D8", textAlign: "center", lineHeight: 1.6, marginBottom: 8 }}>
          Permanently delete this message from{" "}
          <strong style={{ color: "#e3dfff" }}>#{msg.roomName}</strong>?
        </p>
        <div
          style={{
...
function ReportedMessageRow({ msg, onDelete, onWarn, onIgnore, onBan }: ReportedMessageRowProps) {
  ...
  return (
    <div
      style={{
        background: "rgba(255,180,171,.04)",
        border: "1px solid rgba(255,180,171,.15)",
        borderRadius: 18,
        padding: "16px 18px",
        transition: "all .2s",
      }}
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 8 }}>
            ...
            <span
              style={{
                ...
                border: `1px solid ${reasonCfg.color}35`,
            ...
            {(msg.warningCount ?? 0) > 0 && (
              <span
                style={{
                  ...
                  padding: "3px 9px",
                  borderRadius: 999,
                  background: "rgba(255,183,133,.14)",
                  color: "#FFB785",
                  fontSize: 10,
                  fontWeight: 800,
                }}
          ...
          >
            <span>
              Reported by <strong style={{ color: "#E4E1EE" }}>{msg.reportedBy}</strong>
            </span>
...
function CommunityRoomsAdmin({ toast }: { toast: (m: string, t?: ToastMsg["type"]) => void }) {
  const {
    data: roomsData,
    loading: roomsLoading,
    error: roomsError,
    refetch,
  } = useCollection<ChatRoom>("chat_rooms", [orderBy("createdAt", "desc")]);

  const rooms = (roomsData || []) as ChatRoom[];

  // Client-side sort: pinned first (đã sửa dùng toMillis)
  const sortedRooms = useMemo(() => {
    const pinned = rooms.filter((r) => r.pinned);
    const unpinned = rooms.filter((r) => !r.pinned);
    pinned.sort((a, b) => toMillis(b.lastMessageAt) - toMillis(a.lastMessageAt));
    unpinned.sort((a, b) => toMillis(b.lastMessageAt) - toMillis(a.lastMessageAt));
  ...
  const { data: messagesData, loading: msgLoading } = useCollection<ChatMessage>(
    `chat_rooms/${msgRoom?.id || "dummy"}/messages`,
    [orderBy("timestamp", "desc"), limit(100)],
    [msgRoom?.id]
  );
  const roomMessages = (messagesData || []) as ChatMessage[];
  ...
  const filtered = useMemo(() => {
    ...
    return r;
  }, [sortedRooms, search, filterType, filterActive]);

  const handleSaveRoom = async (form: EditRoomForm, id?: string) => {
    if (id) {
      await updateRoom(id, { ...form });
      toast(`Room "${form.name}" updated ✓`);
    } else {
      const userId = "admin_001"; // TODO: get from AuthContext
      await createRoom({ ...form, createdBy: userId });
      toast(`Room "#${form.name}" created ✓`);
    }
  ...
  const handleDeleteRoom = async (room: ChatRoom) => {
    ...
    toast(`Room "${room.name}" deleted (inactive)`, "error");
    refetch();
  };

  const handleDeleteMsg = async (msg: ChatMessage) => {
    if (!msgRoom) return;
    await deleteMessage(msgRoom.id, msg.id);
    toast("Message deleted from Firestore", "error");
    setDeleteTarget(null);
  };

  const totalReported = rooms.reduce((s, r) => s + r.reportedCount, 0);
  if (roomsError) return <div style={{ textAlign: "center", padding: 40, color: "#ffb4ab" }}>Error loading rooms: {roomsError.message}</div>;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              color: "#fff",
              boxShadow: "0 0 18px rgba(108,99,255,.28)",
            }}
          >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
        {[
          ...
          {
            label: "Total Members",
            val: fmtNum(rooms.reduce((s, r) => s + r.memberCount, 0)),
            color: "#FFB785",
            glow: "rgba(255,183,133,.08)",
          },
          {
            label: "Reported Msgs",
            val: totalReported,
            color: totalReported > 0 ? "#ffb4ab" : "#47464f",
            glow: totalReported > 0 ? "rgba(255,180,171,.1)" : "rgba(0,0,0,.04)",
          },
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        ...
        {(["all", "general", "study", "course", "announcement"] as const).map((t) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              transition: "all .15s",
              background: filterType === t ? "linear-gradient(135deg,#6C63FF,#9B59B6)" : "rgba(255,255,255,.04)",
              border: filterType === t ? "1px solid rgba(108,99,255,.35)" : "1px solid rgba(255,255,255,.08)",
              color: filterType === t ? "#fff" : "#C7C4D8",
            }}
          >
        ...
        <button
          onClick={() => setFilterActive((p) => (p === "all" ? true : p === true ? false : "all"))}
          style={{
            padding: "8px 14px",
            borderRadius: 10,
            fontSize: 12,
            fontWeight: 700,
            cursor: "pointer",
            background: filterActive !== "all" ? "rgba(69,241,197,.12)" : "rgba(255,255,255,.04)",
            border: filterActive !== "all" ? "1px solid rgba(69,241,197,.3)" : "1px solid rgba(255,255,255,.08)",
            color: filterActive !== "all" ? "#45f1c5" : "#C7C4D8",
          }}
        >
        ...
        <span style={{ marginLeft: "auto", fontSize: 12, color: "#C7C4D8", alignSelf: "center" }}>
          {roomsLoading ? "Loading…" : `${filtered.length} rooms`}
        </span>
      </div>
      ...
      {roomsLoading ? (
        ...
        >
          <MessageSquare size={40} color="#47464f" style={{ margin: "0 auto 14px" }} />
          <p style={{ fontSize: 16, fontWeight: 700, color: "#E4E1EE" }}>No rooms found</p>
          <p style={{ fontSize: 13, color: "#C7C4D8" }}>Click "New Room" to create your first chat room.</p>
        </div>
      ) : (
        filtered.map((room) => (
          <RoomCard
      ...
      {formRoom !== undefined && <RoomFormDialog room={formRoom} onSave={handleSaveRoom} onClose={() => setFormRoom(undefined)} />}
      {msgRoom && (
        <MessagePreviewModal
          room={msgRoom}
          messages={roomMessages}
          loading={msgLoading}
          onClose={() => setMsgRoom(null)}
          onDeleteMsg={(msg) => setDeleteTarget(msg)}
        />
      )}
      {deleteTarget && <DeleteConfirmDialog msg={deleteTarget} onConfirm={() => handleDeleteMsg(deleteTarget)} onCancel={() => setDeleteTarget(null)} />}
    </div>
  );
}
...
function ReportedMessagesAdmin({ toast }: { toast: (m: string, t?: ToastMsg["type"]) => void }) {
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState<string>("all");
  const [deleteTarget, setDeleteTarget] = useState<ChatMessage | null>(null);

  // FIX: lấy roomId từ doc.ref.parent.parent?.id
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snap) => {
        const msgs = snap.docs.map((doc) => {
          const data = doc.data();
          const roomId = doc.ref.parent.parent?.id; // chat_rooms/{roomId}/messages
          return {
            id: doc.id,
            roomId: roomId,
  ...
  const filtered = useMemo(() => (filter === "all" ? messages : messages.filter((m) => m.reportReason === filter)), [messages, filter]);

  const handleDelete = async (msg: ChatMessage) => {
    await deleteMessage(msg.roomId, msg.id);
  ...
  const handleWarn = async (msg: ChatMessage) => {
    await warnUser(msg.userId, `Warning from admin for message ${msg.id}`);
    await ignoreReport(msg.roomId, msg.id);
    setMessages((prev) => prev.filter((m) => m.id !== msg.id));
  ...
  const handleBan = async (msg: ChatMessage) => {
    await banUser(msg.userId, `Ban due to reported message: ${msg.reportReason}`);
    await ignoreReport(msg.roomId, msg.id);
    setMessages((prev) => prev.filter((m) => m.id !== msg.id));
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 14 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "#C7C4D8" }}>Firestore: collectionGroup messages where isReported == true</p>
        </div>
        <div
          style={{
            display: "flex",
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button
          ...
          style={{
            ...
            fontWeight: 700,
            cursor: "pointer",
            background: filter === "all" ? "linear-gradient(135deg,#6C63FF,#9B59B6)" : "rgba(255,255,255,.04)",
            border: filter === "all" ? "1px solid rgba(108,99,255,.3)" : "1px solid rgba(255,255,255,.08)",
            color: filter === "all" ? "#fff" : "#C7C4D8",
          }}
        >
          All ({messages.length})
        </button>
          ...
          return (
            <button
              ...
              style={{
                ...
                fontWeight: 700,
                cursor: "pointer",
                background: filter === r ? cfg.bg : "rgba(255,255,255,.04)",
                border: `1px solid ${filter === r ? cfg.color + "50" : "rgba(255,255,255,.08)"}`,
                color: filter === r ? cfg.color : "#C7C4D8",
              }}
            >
              {cnt} {cfg.label}
            </button>
      ...
      {deleteTarget && <DeleteConfirmDialog msg={deleteTarget} onConfirm={() => handleDelete(deleteTarget)} onCancel={() => setDeleteTarget(null)} />}
    </div>
  );
}
  ...
  return (
    <div
      style={{
        ...
        color: "#E4E1EE",
        fontFamily: "Inter,sans-serif",
        backgroundImage: "radial-gradient(circle at 5% 0%, rgba(108,99,255,.06) 0%, transparent 55%)",
      }}
    >
      <style>{`
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "28px 24px", display: "flex", flexDirection: "column", gap: 22 }}>
        ...
        >
          {[
            { id: "rooms", label: "Chat Rooms", Icon: Hash },
            { id: "reports", label: "Reported Messages", Icon: Flag },
          ].map(({ id, label, Icon }) => (
            <button
              key={id}
              onClick={() => setTab(id)}
              style={{
                ...
                cursor: "pointer",
                transition: "all .2s",
                background: tab === id ? "linear-gradient(135deg,#6C63FF,#9B59B6)" : "transparent",
                border: tab === id ? "1px solid rgba(108,99,255,.3)" : "1px solid transparent",
                color: tab === id ? "#fff" : "#C7C4D8",
                boxShadow: tab === id ? "0 0 16px rgba(108,99,255,.25)" : "none",
              }}
            >
              <Icon size={14} /> {label}
            </button>
        ...
        <div style={{ animation: "fadeDown .25s ease" }}>
          {tab === "rooms" ? <CommunityRoomsAdmin toast={toast} /> : <ReportedMessagesAdmin toast={toast} />}
        </div>
      </div>
